...
| D4 | Pollution | Strategy **(B) CO$_2$ as indicator** for the baseline: $P$ is cumulative fossil CO$_2$, $\Xi$ maps to it through the fossil share of the outflow, damages from a conventional integrated-assessment loss at a reference stock. Strategy (A) is a variant, task B5 records what it would take. |
| D5 | Units | Goods in trillions of 2011 international dollars, material in gigatonnes, one period one year, pollution in gigatonnes CO$_2$. `quant_model.tex` §*Period length and units* is updated to say so (E2). |
