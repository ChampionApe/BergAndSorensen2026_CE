...
\smalltitle{What is kept and what is dropped.} Everything in the planner's setup and optimisation problem of \theorynote{} is kept: six states, the full materials accounting, both corner-prone recycling margins, the extraction and exploration corners, the material floor and the yield ceiling in either case. The parameter space is that of \theorynote{}: the handling share $\mu\in\left(0,1\right]$, the floor $\bar R\ge0$ and the yield ceiling $\bar a\in\left(0,1\right]$, with $\bar a<1$ the hard ceiling and $\bar a=1$ the soft one. Technology trends throughout, which is the maintained regime of the classification of \theory{Section}{The long run}; stationary technology, $g_A=g_B=0$ with the terminal closure of Section~\ref{sec:q:sol:terminal} at $\Gamma=1$, is a benchmark outside that classification and is used for the benchmark checks of Section~\ref{sec:q:sol:verification} alone. Nothing is linearised and no margin is assumed interior. Three simplifications are made, all recorded so they can be relaxed.
\begin{itemize}
...
\smalltitle{Period length and units.} A period is $\Delta$ years, with $\Delta=1$ the default; every rate parameter ($\rho,\delta,\mu,\theta,g_A,g_B$) is a per-period rate and is converted from an annual rate as $1-\left(1-\text{annual}\right)^{\Delta}$ for decay shares and $\Delta\times$ for log growth rates. Goods are measured in trillions of 2011 international dollars, material in gigatonnes, and pollution in gigatonnes of material that has reached the environment and has not regenerated; the two Kuhn--Tucker margins are unit-free, and every shadow price is in dollars per tonne, exactly as in \theory{Section}{Units}. The baseline is annual, $\Delta=1$, and runs from $t=0$ at 1900 to a terminal date far beyond the horizon of interest, for reasons Section~\ref{sec:q:sol:horizon} explains.
