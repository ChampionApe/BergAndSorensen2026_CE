...
Preferences &
$\rho,\eta$ &
long-run real interest rate and the consumption--wealth ratio; $\eta$ additionally restricted by the well-posedness condition $\ln\left(1+\rho\right)>\left(1-\eta\right)g$ and, jointly with $\delta$, by the collapse ranking condition $\left(1-\delta\right)^{1-\eta}<1+\rho$, both of \theory{Section}{Scope: maintained long-run assumptions} \\
Damages &
$\kappa,\psi,\varphi$ &
...
\smalltitle{(E1) Where is the economy in the taxonomy?} Solve the planner's problem on the baseline calibration and locate the resulting path in the table of \theory{Section}{Taxonomy}, that is, which of the three states it reaches, collapse, balanced dematerialization or perpetual circular growth: does the loop close, and is the material era bounded. Report the retained endowment $\mathcal M_\infty$, the turnover time $\mathcal T$ and the survival ratio $\mathcal M_\infty/\left(\bar R\mathcal T\right)$, which is the single number the theory says matters, together with the horizon diagnostics of Section~\ref{sec:q:sol:horizon} and the date the gate fee changes sign.

\smalltitle{(E2) How much does circularity buy in the cells where it is not survival?} Wherever the long run is collapse or balanced dematerialization the circular economy is a transition technology, and for the collapse cells \theory{Section}{The shutdown and its aftermath} says exactly what it does: it multiplies the throughput budget without changing the slope of the material price path. Measure the resulting welfare gain in consumption equivalents against a no-recycling counterfactual, the cumulative pollution avoided, and, where the material era ends within the horizon, its extension, as a function of $\bar a$ and $\xi$. The slope statement is about the relaxed problem of that section and is measured only where the reserve carries no stock effect and exploration has ceased.

\smalltitle{(E3) The cost of the three market failures.} Solve the market economy at each corner of the policy space $\left(\phi^W,\phi^z,\phi^P,\phi^X\right)$ and along paths between them. Two questions are separable and both are interesting: the conventional one, how much welfare each instrument is worth, and the one specific to this model, whether any combination of missing instruments moves the economy across the survival margin of \theory{Section}{Laissez-faire and the survival margin}. A policy failure that costs a few percent of consumption is a familiar object; one that converts perpetual growth into a scheduled shutdown is not, and whether the second is possible on any defensible calibration is the paper's sharpest quantitative question.
...
\smalltitle{(E5) Sensitivity along the two axes that history cannot pin down.} Report the state and the survival ratio of E1 as a surface over $\left(\bar R,\bar a\right)$, over the floors the solver reaches, since these are the parameters the taxonomy turns on and the parameters the data cannot deliver.
