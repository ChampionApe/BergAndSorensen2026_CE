...
\end{align}
where $H$ is the \emph{handled flow}: the tonnage collected and processed per period. Everything downstream of generation is levied on $H$ rather than on $W$, and because $H_t$ is drawn from the predetermined stock, the feedstock available for handling in period $t$ is fixed before any period-$t$ choice is made. A tonne's residence time in the stock is geometric with mean $1/\mu$ periods. We treat the handling share $\mu$ as a technological constant; together with the timing convention it prevents waste generated in a period from being recycled within that same period, and it gives the stockpile the persistence that avoids indeterminate or degenerate solutions in the long run.\footnote{The corner $\mu=1$ is the one-period buffer $\mathcal W_{t+1}=W_t$: all waste is handled exactly one period after it is generated. It is a legitimate simplification for the quantitative implementation, but the theory maintains $\mu<1$, which keeps the stock from emptying in finite time ($\mathcal W_t\ge\left(1-\mu\right)^t\mathcal W_0>0$) and preserves the stockpile as a genuinely persistent asset.}
