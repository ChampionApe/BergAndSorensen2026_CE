...
\smalltitle{Time.} Time is discrete, $t\in\left\{0,1,2,\dots\right\}$. Flows dated $t$ occur during period $t$; stocks dated $t$ are measured at the beginning of period $t$, so that each stock's transition equation states what the period-$t$ flows leave behind for period $t+1$. Where no confusion arises we suppress the period subscript and write it only where timing carries content, chiefly in the transition equations; the explicit $t$ argument in the primitives below represents exogenous technological change. The per-period rates $\delta$ and $\theta$ introduced below lie in $\left(0,1\right)$, and the handling share $\mu$ in $\left(0,1\right]$.
...
\begin{align}
U_0 &= \sum_{t=0}^{\infty}\beta^t\left[u(C_t)-v(P_t)\right],
&&
\beta\equiv\frac{1}{1+\rho},
&&
...
Physical waste flows $W$ accumulate in a stock $\mathcal{W}$ of waste held within the economy.\footnote{We think about landfills, stockpiles, discarded durables etc.} Every waste stream generated during period $t$ enters the stock at $t+1$, and each period a share $\mu$ of the beginning-of-period stock is drawn out for handling:
\begin{align}
\mathcal W_{t+1} &= \left(1-\mu\right)\mathcal W_t+W_t,
&
H_t &\equiv \mu\mathcal W_t,
&&
\mathcal W_0\text{ given},
\label{eq:sp:setup:wastestock}
\end{align}
where $H$ is the \emph{handled flow}: the tonnage collected and processed per period. Everything downstream of generation is levied on $H$ rather than on $W$, and because $H_t$ is drawn from the predetermined stock, the feedstock available for handling in period $t$ is fixed before any period-$t$ choice is made. A tonne's residence time in the stock is geometric with mean $1/\mu$ periods. We treat the handling share $\mu$ as a technological constant; it is the timing convention, under which waste generated in period $t$ is handled only from $t+1$ onward, that prevents waste from being recycled within the period in which it is generated and so removes any within-period fixed point in the material block.\footnote{The corner $\mu=1$ is the one-period buffer $\mathcal W_{t+1}=W_t$: all waste is handled exactly one period after it is generated. Every result in this note holds there. What $\mu<1$ adds is a stock that is a distinct object from the flow feeding it, and a mean residence time $1/\mu$ to calibrate.}
...
\smalltitle{Waste collection and treatment.} Handled waste is either dumped, which is free, or collected and treated: the treated share $\varpi$ of the handled flow is charged the collection cost $c^c$ per tonne and carries an increasing marginal treatment cost on top:
\begin{align}
C^W &= \left[c^c(t)\,\varpi+c^T(\varpi,t)\right]H,
&&
...
\smalltitle{Accumulation.} The aggregate resource constraint and transition equations for the remaining stocks are:
\begin{align}
Y &= C+I+\delta K+C^N+C^D+C^W, \label{eq:sp:setup:resource-constraint} \\
K_{t+1} &= K_t+I_t, \qquad S_{t+1}= S_t+D_t-N_t, \qquad X_{t+1} = X_t+D_t \label{eq:sp:setup:reserves-motion} \\
P_{t+1} &= P_t+\Xi_t-\theta(P_t)P_t, \qquad \theta'(P)<0, \quad \theta(P)\in\left(0,1\right),
\label{eq:sp:setup:pollution-motion}
\end{align}
together with the waste stock's transition~\eqref{eq:sp:setup:wastestock}, where the flow of waste that actually reaches the environment is\footnote{Making the stockpile environmentally free while it sits is a modelling choice, not a fact; a leakage term $\ell\mathcal W_t$ added to the pollution transition would give landfills a flow cost alongside their asset value, and is recorded in Appendix~\ref{sec:app:extensions}.}
\begin{align}
...
\begin{align}
M^K_{t+1} &= \left(1-\delta\right)M^K_t+\Phi^I(t)\left(I_t+\delta K_t\right),
&
M^K_0 \text{ given},
\label{eq:sp:setup:Phi-motion}
\end{align}
structurally identical to $K$'s own transition written in gross form, $K_{t+1}=\left(1-\delta\right)K_t+\left(I_t+\delta K_t\right)$, with the flow intensity $\Phi^I(t)$ replacing the constant $1$ on gross investment. Equivalently, in ratio form,
\begin{align}
\Phi^K_{t+1}-\Phi^K_t &= \left[\Phi^I(t)-\Phi^K_t\right]\frac{I_t+\delta K_t}{K_{t+1}},
\label{eq:sp:setup:phiK-motion}
\end{align}
so $\Phi^K_t$ is an investment-weighted moving average of past values of $\Phi^I$: it drifts toward the current vintage's intensity at a rate set by the gross investment rate, and is stationary exactly when $\Phi^I(t)=\Phi^K_t$.
...
Finally, the material intensity of new capital goods, $\Phi^I$, can be interpreted in different ways: We can assume that it requires a very specific level of materials to produce durables, which then requires $\Phi^I$ to be exogenous in levels, we can even assume that this level is constant over time (simplifying the model by removing a state variable), or we may assume that the material intensity of durables follows that of the final good (in levels at least). The documentation uses this last assumption and assumes $\Phi^I = \Omega \phi^I$; the other two versions are set aside in Appendix~\ref{sec:app:extensions}. Under this assumption we have:
\begin{align}
...
\end{align}
with $\mathcal D$ collecting flows measured in final-good units, carrying embodied material drawn through $R$, while $\mathcal N$ gathers the two physical flows that do not, and with $C^W=c^WH$, $c^W\equiv c^c\varpi+c^T(\varpi)$ throughout. Given this additional notation, our first "result" comes from substituting~\eqref{eq:sp:setup:ybalance_generic2} into the waste sum~\eqref{eq:sp:setup:totalwaste_generic} to get:\footnote{The substitution and sums cancels $\Omega$ and every $\omega^j$ from the accounted part, and~\eqref{eq:sp:setup:Phi-motion} collects the two remaining capital terms into $\Delta M^K$. Throughout, $\Delta Z_{t+1}\equiv Z_{t+1}-Z_t$ denotes the per-period change of a stock $Z$.}
\begin{align}
\boxed{\;W_t \;=\; R_t-\Phi^IG_t+\delta M^K_t+\mathcal N_t \;=\; R_t-\Delta M^K_{t+1}+\mathcal N_t\;}
...
Next, substituting $R=N+a\varpi H$ into~\eqref{eq:sp:setup:ledger} and using $W_t=\Delta\mathcal W_{t+1}+H_t$ from~\eqref{eq:sp:setup:wastestock} therefore gives
\begin{align}
\left(1-a_t\varpi_t\right)H_t &= N_t-\Delta M^K_{t+1}-\Delta\mathcal W_{t+1}+\mathcal N_t,
&
...
The factor $\left(1-a\varpi\right)^{-1}$ is the \emph{circularity multiplier}: the number of times a tonne of material passes through production before it finally leaves the economy. With the waste stock in place it is a steady-state object rather than a period-by-period identity: whenever the material block is stationary ($\Delta\mathcal W=\Delta M^K=0$, so that $H=W$) and no mass is displaced ($\mathcal N=0$), \eqref{eq:sp:setup:ledger-collected} reads $N=W\left(1-a\varpi\right)$ --- virgin extraction equals waste not recycled --- and each tonne entering the economy serves production $\left(1-a\varpi\right)^{-1}$ times on its way through: literally the sum of the geometric series in which a fraction $a\varpi$ of each pass through production returns for another. A fully circular economy --- one that extracts nothing while processing a positive throughput --- requires $a\varpi=1$, and whether that is out of reach is exactly the content of the yield-ceiling case distinction.
...
Under the soft ceiling~(S) no such uniform bound exists. The multiplier is finite at every date --- $a\left(x,t\right)<1$ for every finite $x$, and Assumption~\eqref{eq:sp:setup:cT-finite} does not help, since $\varpi\le1$ --- but it is unbounded along paths on which recycling capital per treated tonne diverges, so full circularity is approached rather than attained. What disciplines throughput in this case is the waste stock itself. Recovered material is drawn from the handled flow, so in every period and under either ceiling
\begin{align}
R^R_t &= a_t\varpi_t\,\mu\mathcal W_t \;\le\; \bar a\,\mu\mathcal W_t,
&
R_t &\le N_t+\bar a\,\mu\mathcal W_t ,
\label{eq:sp:setup:throughput-cap}
\end{align}
and the bound binds on a stock that is predetermined at the start of the period. \emph{Circular throughput is bounded by the retained stock of waste times the handling share.} However close $a\varpi$ comes to one, a closed loop cannot manufacture throughput out of nothing: sustaining a recycled supply $R^R$ requires holding an anthropogenic reserve of at least $R^R/\left(\bar a\mu\right)$ tonnes. The multiplier measures how far a tonne stretches over its lifetime in the economy; the stock and the handling share cap how fast material can circulate in any one period. The economically meaningful long-run question under the soft ceiling is then whether the leakage the multiplier measures is cumulatively finite, and Section~\ref{sec:sp:longrun} works with the form~\eqref{eq:sp:setup:ledger-collected} throughout, since it is the one statement of the ledger that does not presume which ceiling case holds.
