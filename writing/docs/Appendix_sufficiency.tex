...
\emph{(ii) The long-run states need optimality of a candidate path.} States A, B and C are candidate configurations verified against the necessary conditions. To call them optimal we need a sufficiency theorem plus transversality.
...
\end{align}
On every candidate path of Section~\ref{sec:sp:longrun}, both requirements hold under the explicit conditions of Assumption~\ref{ass:lr:wellposed}, given the convergence of Assumption~\ref{ass:lr:convergence}, and under no further condition.
\end{proposition}

The bounds are Lemma~\ref{lem:lr:stocks} and~\eqref{eq:sp:sum:cumulative-N} together with $\theta\ge\theta_{\min}$ applied to a cumulated emission flow that is itself bounded by the budget. The verification of the limits is a rate count, with one case for each kind of candidate path that Assumption~\ref{ass:lr:convergence} admits. On a balanced path (states B and C) $\lambda_t\propto e^{-\eta gt}$ and $K_{t+1}\propto e^{gt}$, so the capital term~\eqref{eq:app:suff:tvc-K} behaves like $\left[\beta e^{\left(1-\eta\right)g}\right]^t$, which converges to zero exactly when $\ln\left(1+\rho\right)>\left(1-\eta\right)g$, the well-posedness condition of Assumption~\ref{ass:lr:wellposed}. The five bounded states need less: only $\beta^t\lambda_t p_t\to0$ for each of their prices. On a balanced path each of these prices grows at a constant rate and admits its forward representation, so $\beta^t\lambda_t p_t$ is the tail of a convergent discounted sum, and the regularity inequalities of Assumption~\ref{ass:lr:wellposed} are exactly what makes those sums converge at the balanced rates; the tail therefore vanishes. On a collapse path (state A) the limits are automatic for the five bounded states, since after the shutdown each of their prices is a discounted sum of damages along the legacy emission stream, which vanishes as $P\to0$; the capital term behaves like $\left[\beta\gamma_C^{1-\eta}\right]^t$ along the cake-eating aftermath of Appendix~\ref{sec:app:proofs}, and vanishes exactly under the ranking condition $\left(1-\delta\right)^{1-\eta}<1+\rho$ that Assumption~\ref{ass:lr:wellposed} carries. Hence:

\begin{center}
\emph{The conditions of Assumption~\ref{ass:lr:wellposed} are not an additional restriction for sufficiency. They are the transversality conditions, one for each kind of candidate path.}
\end{center}
