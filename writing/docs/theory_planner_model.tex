...
\end{align}
with waste bases $\mathcal{N}$ and $\mathcal{D}$ defined in \eqref{eq:sp:setup:base}, $c^W\!\left(\varpi,t\right) \equiv c^c(t)\varpi+ c^T(\varpi,t)$, the definitions in~\eqref{eq:sp:definitions}, the emission flow from \eqref{eq:sp:setup:Xi}, levied on the handled flow $H$, and domains $C,N,D,K^R,W,\Omega\ge0$, $K_{t+1}\ge0$, $\varpi\in\left[0,1\right]$.\footnote{Gross investment $I+\delta K$ may be negative, so that capital can be scrapped for consumption, which the shutdown aftermath of Section~\ref{sec:sp:longrun} uses; on every operating path it is positive.}
\end{problem}
...
never needs a multiplier: with $W\ge0$ the transition in~\eqref{eq:sp:states-motion} delivers
$\mathcal W_{t+1}\ge0$ directly, and the stock is strictly positive on every path of interest, since
$W\ge\Omega\omega^YY>0$ while production runs and $W=\delta M^K>0$ after it stops, so the recycling
block never runs out of feedstock.
