...
$\bar R\ge0$. The two readings of the embodied-materials intensity $\Phi^I$ named in
Section~\ref{sec:sp:setup:aggregatematerial}, exogenous in levels and constant over time, are set
aside and are not written up. What remains here are variants we have not built. Open items of the
long-run analysis itself, as distinct from these variants, are listed in
Section~\ref{sec:sp:longrun:sketches} and are not repeated here.
...
exploration effort, and of treatment is still released as waste in the same period the final good is
...
specification recovered in the limit of immediate release, and the ledger survives unchanged in form
with $M^K$ replaced by total material in use. Second, the handling share $\mu$ as a control --- an
...
leaking stockpile: a term $\ell\mathcal W_t$ in the pollution transition that gives landfills a flow
cost alongside their asset value. The theory keeps $\mu$ constant and the stockpile inert; the
quantitative model is the natural place for the last two margins.
...
\smalltitle{Stationary technology.} The classification of Section~\ref{sec:sp:longrun} maintains
trending technology, so the stationary alternative is not a variant waiting to be built: it is
carried as a benchmark in Appendix~\ref{sec:app:stat}, with its rest points, the essentiality
distinction that organises survival where technology does not trend, and the result that a closed
loop is infeasible in a stationary economy. It sits outside the classification because it decides no
cell of the taxonomy, and it is kept because the quantitative note's terminal closure and
verification use its closed-form rest point.
