...
\item \textbf{A per-period rate lies in $(0,1)$}: $\delta$, $\theta$, $d^W$, with the handling share
$\mu$ in $(0,1]$, its upper corner the one-period buffer $\mathcal W_{t+1}=W_t$. A growth rate, an
...
$\epsilon_a$ & tail elasticity of the yield function, $xa'/(1-a)$ & dimensionless & \eqref{eq:app:wh:cbgp:elasticity}\\
$L$ & per-period leakage rate, $1-a(x)$ & dimensionless & \eqref{eq:app:pf:leakage}\\
...
$\Theta_{\mathcal W}$ & stockpile multiplier on a closed loop, $p^{\mathcal W}=\Theta_{\mathcal W}\Psi$ & dimensionless & \eqref{eq:app:wh:cbgp:multipliers}\\
...
$\mu$ & handling share of the waste stock, $\mu\in\left(0,1\right]$ & rate & \eqref{eq:sp:setup:wastestock}\\
...
\multicolumn{4}{l}{\textit{The long run}}\\
$\mathcal B$ & material budget: all mass that can ever enter circulation & tonnes & \eqref{eq:sp:lr:budget}\\
$\mathcal M_\infty$ & retained material endowment, $\lim_t\left(M^K_t+\mathcal W_t\right)\le\mathcal B$ & tonnes & \eqref{eq:sp:lr:little}\\
$\mathcal T$ & mean residence time of a tonne per pass, $1/\mu+\sigma_\infty/\delta$ & periods & \eqref{eq:sp:lr:little}\\
A, B, C & long-run states: collapse, dematerialization, circular growth & --- & Definitions~\ref{def:lr:collapse}--\ref{def:lr:circular}\\
\hline
\end{tabular}
\end{center}
