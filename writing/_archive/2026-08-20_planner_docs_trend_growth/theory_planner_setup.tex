...
\smalltitle{Final goods.} Output is produced from capital and raw materials, is reduced by
pollution, and is subject to exogenous technological progress:
\begin{align}
Y &= A\,F\left(K^Y,BR,P\right),
&&
...
B=e^{g_Bt},
&&
g_A\ge0,\quad g_B\ge0,
\label{eq:sp:setup:tfp}
\end{align}
with the initial levels $A(0)=B(0)=1$ a normalisation absorbed into $F$. The two trends are
distinct in kind. $A$ is Hicks-neutral total factor productivity: it scales output and both
marginal products equally and carries no material content. $B$ is \emph{material-augmenting}
progress: a physical tonne delivers $B$ units of material \emph{services} to production, so a
growing $B$ is dematerialisation of the technology --- more output per tonne --- never creation
of mass. For constant-returns kernels the pair spans every factor-augmenting configuration,
since capital-augmenting progress is then the combination of the two
($F\left(e^{gt}K,R\right)=e^{gt}F\left(K,e^{-gt}R\right)$); Hicks-neutral progress alone is
$g_B=0$, resource-augmenting progress alone is $g_A=0$. The stationary technology of the
earlier drafts is the case $g_A=g_B=0$, i.e.\ $A=B=1$ throughout.

\smalltitle{Notation for marginal products.} Throughout the documentation, $F_K$, $F_R$ and
$F_P$ denote the derivatives of \emph{output} with respect to the physical inputs,
\begin{align}
F_K\equiv\frac{\partial Y}{\partial K^Y}=A\,F_1,
\qquad
F_R\equiv\frac{\partial Y}{\partial R}=AB\,F_2,
\qquad
F_P\equiv\frac{\partial Y}{\partial P}=A\,F_3,
\label{eq:sp:setup:mpconvention}
\end{align}
with the trend factors absorbed (subscripts $1,2,3$ denoting partials of the kernel $F$ by
argument). The sign restrictions in~\eqref{eq:sp:setup:output} are unaffected, and every
optimality condition below is form-invariant under this convention: technology growth enters
the system only through the drift it puts into these marginal products, never through a new
term of its own.

Two conventions attached to the trends are worth making explicit. First, the trends live in
goods production only; the material-handling technologies below --- the recycling yield, the
extraction, discovery, collection and treatment costs --- are held stationary \emph{in
final-good units}. Because those costs are denominated in the final good, this is not an
assumption of technological stagnation in the loop: handling a tonne claims a fixed goods
bill while goods themselves become ever cheaper to produce, so the material-handling side
shares Hicks-neutral progress one for one. What the convention rules out is
\emph{differential} progress --- a recycling yield or an extraction technology improving
faster or slower than final goods --- which we leave as an extension. Second, everywhere in
the materials accounting of Sections~\ref{sec:sp:setup:materialaccounting}--\ref{sec:sp:setup:aggregatematerial},
$R$ remains the physical tonnage entering production: $B$ multiplies services inside $F$ and
appears nowhere in a mass balance.
...
\end{align}
Write $\varepsilon\equiv a'x/a$ for the elasticity of the yield function. It is convenient to have this technology available in constant-returns form, since both model
...
Next, substituting $R=N+a\varpi W$ into~\eqref{eq:sp:setup:ledger} and collecting therefore gives
\begin{align}
