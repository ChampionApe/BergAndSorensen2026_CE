...
\end{align}
with waste bases $\mathcal{N}$ and $\mathcal{D}$ defined in \eqref{eq:sp:setup:base}, $c^W\!\left(\varpi,t\right) \equiv c^c(t)+ c^T(\varpi,t)$, the definitions in~\eqref{eq:sp:definitions}, the emission flow from \eqref{eq:sp:setup:Xi}, levied on the handled flow $H$, and domains $C,I+\delta K,N,D,K^R,W,\Omega\ge0$, $\varpi\in\left[0,1\right]$.
\end{problem}
...
={}& u(C)-v(P)
\notag\\
&+\lambda\Big[F\left(K-K^R,R,P,t\right)-C-I-\delta K-C^N(N,S,t)-C^D(D,X,t)-c^W\!\left(\varpi,t\right)\mu\mathcal W\Big]
\notag\\
&+\lambda q\,I
...
\;+\;\lambda p^{\mathcal W}\left[W-\mu\mathcal W\right]
\notag\\
...
\emph{Waste.} At the instant of generation a tonne of waste has exactly one consequence: it enters the stockpile. Collection, treatment, recycling and emissions are all levied on the handled flow $H=\mu\mathcal W$ and are therefore priced on the state, not on the control, so the waste margin collapses to an identification:
\begin{align}
p^W &= -p^{\mathcal W} .
\label{eq:sp:sum:W}
\end{align}
The social cost of generating a tonne of waste is minus the value of a tonne in the anthropogenic reserve. Everything levied on handling --- resources absorbed, emissions, feedstock recovered --- sits in the stockpile costate~\eqref{eq:sp:sum:Wstock} below, and waste generation is a social cost precisely when the stockpile is a liability.
...
never needs a multiplier: with $W\ge0$ the stock obeys $\mathcal W\left(t\right)\ge\mathcal W_0e^{-\mu t}>0$,
...
and mutually exclusive, and the interior root is unique when it exists. The upper corner is reachable
...
\begin{align}
\int_0^\infty N\,dt \;\le\; S_0+\bar X_{\max}-X_0,
...
\emph{The anthropogenic reserve.} A tonne in the stockpile is drawn for handling at rate $\mu$, and each handled tonne yields the \emph{handling dividend}
\begin{align}
...
\end{align}
The stockpile is priced exactly as the virgin reserve is: $S$ pays its dividend by making future extraction cheaper, $\mathcal W$ by making future recovery possible, and both prices are forward integrals of those dividends. Two readings of~\eqref{eq:sp:sum:Wstock} are worth recording. First, substituting $p^W=-p^{\mathcal W}$ into $h$ and collecting terms shows that the effective discount rate on the stockpile is $r+\mu\left(1-\alpha\varpi\right)$: in marginal-value terms the stockpile runs off only through genuine leakage, because the recovered fraction $\alpha\varpi$ of each handled tonne re-enters production and eventually returns to the stockpile as waste. Second, at a rest point of the price system --- and, more generally, in the instantaneous-handling limit $\mu\to\infty$ --- \eqref{eq:sp:sum:Wstock} gives $p^{\mathcal W}=\mu h/\left(r+\mu\right)\to h$, and combining with~\eqref{eq:sp:sum:W} recovers the static waste margin
\begin{align}
...
\end{align}
the condition a model without a waste stock would impose at every instant. The waste stock changes \emph{when} the waste price is paid, not what it prices in steady state.
