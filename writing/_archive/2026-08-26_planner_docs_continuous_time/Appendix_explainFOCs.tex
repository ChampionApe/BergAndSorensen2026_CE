...
\item \emph{Tonnes.} Raw material $R$ and its components $N$ and $R^R$, waste generation $W$, the waste stock $\mathcal W$ and the handled flow $H=\mu\mathcal W$ (the handling rate $\mu$ carrying units of inverse time), treated waste $T=\varpi H$, the reserve $S$, cumulative discoveries $X$ and their ceiling $\bar X_{\max}$, the discovery flow $D$, embodied material $M^K$, the pollution stock $P$, the emission flow $\Xi$, and the nature-attributable base $\mathcal N$.
...
\item \emph{Dimensionless.} The relative intensities $\omega^j$, the durables coefficient $\phi^I$, the recycling functions $a$, $\alpha$ and $\varepsilon$ and the yield ceiling $\bar a$, the leakage rate $d^W$, the treatment share $\varpi$, the storage share $\sigma$, the marginal product of capital $F_K$, the shadow price of capital $q$, and the nature-attributable intensities $\Omega^{N,S}$ and $\Omega^{D,S}$.
\end{itemize}
...
One notational convention is carried over from Section~\ref{sec:sp:setup:primitives}: the primitives $F$, $a$, $C^N$, $C^D$, $c^c$ and $c^T$ all shift exogenously with calendar time, and the argument $t$ is suppressed throughout this appendix. Nothing below depends on the suppression. Since $t$ is not a choice variable it contributes no first-order condition of its own, and every condition holds at each instant with the primitives evaluated at that date. Technological change matters for how the system evolves --- the problem is non-autonomous --- not for the form of the conditions that characterise it.
...
Exhaustibility acts on this margin, and only on this margin. By~\eqref{eq:sp:setup:exhaustible-discovery} the marginal cost of the first tonne, $C^D_D\left(0,X,t\right)$, diverges as $X\to\bar X_{\max}$, while the left-hand side of~\eqref{eq:sp:foc:D} stays bounded: $p^X\le0$, and $p^S$ is finite along any admissible path. So~\eqref{eq:sp:foc:D} can hold with $D>0$ only while $X$ is strictly below its ceiling; exploration must eventually cease or dwindle to nothing, and $X$ approaches $\bar X_{\max}$ at most asymptotically. No constraint $X\le\bar X_{\max}$ ever has to be adjoined to the problem --- the cost function polices the ceiling by itself --- and cumulative virgin extraction inherits the bound, $\int_0^\infty N\,dt\le S_0+\bar X_{\max}-X_0$, as recorded at~\eqref{eq:sp:sum:cumulative-N}.

\smalltitle{Waste.} This is the condition that identifies $p^W$, and with the waste stock it is the simplest condition in the system. At the instant of generation a marginal tonne of waste does exactly one thing: it enters the stockpile. Collection and treatment, recycling, and emissions are all levied on the handled flow $H=\mu\mathcal W$, which is a function of the state rather than of the control, so $W$ appears in the Lagrangian~\eqref{eq:sp:lagrangian} in two places only --- the ledger term $+\lambda p^WW$ and the stock's law of motion $+\lambda p^{\mathcal W}W$ --- and
\begin{align}
\underbrace{p^W}_{\text{(a)}} &= \underbrace{-p^{\mathcal W}}_{\text{(b)}} .
\label{eq:sp:foc:W}
\end{align}
Both sides are in final goods per tonne. Term (a) is the social cost the ledger attributes to generating a tonne; term (b) says that this cost is nothing but the negative of the value of the tonne where it lands. Generating waste is neither taxed for its handling nor credited for its feedstock at this margin --- both lie in the future, at the dates the stockpile releases the tonne --- and they enter through the costate equation~\eqref{eq:sp:costate:Wstock} below, which prices the stockpile as an asset.
...
\smalltitle{Treatment share.} The treatment share is dimensionless, so a perturbation $d\varpi$ routes a further $H\,d\varpi$ tonnes to treatment, of which $\alpha H\,d\varpi$ becomes secondary material, at marginal cost $c^{T\prime}\left(\varpi\right)H\,d\varpi$ final goods. Dividing $\partial\mathcal L/\partial\varpi$ through by $\lambda H$ to state the condition per handled tonne, the marginal gain is
\begin{align}
...
What does \emph{not} follow is an unconditional sign for either price. The sign of $p^W$ is an outcome of the stockpile valuation~\eqref{eq:sp:costate:Wstock} through the identification~\eqref{eq:sp:foc:W}, as set out above. Nor does $p^M$ simply inherit whichever sign $p^W$ currently has: the costate equation~\eqref{eq:sp:costate:M} below makes it a \emph{forward} average of $p^W$, not a function of its current value, so the three signs need not move together along a transition.

\subsection{Costate equations}\label{sec:app:sp:costates}

Each state $Z$ carries the current-value costate $m$ named in~\eqref{eq:sp:costates-def}, obeying $\dot m=\rho m-\partial\mathcal L/\partial Z$, together with the transversality condition $\lim_{t\to\infty}e^{-\rho t}m Z=0$. (We write the generic costate as $m$ rather than the customary $\mu$, which now denotes the handling rate of the waste stock.) Throughout, $r\equiv\rho-\dot\lambda/\lambda$ denotes the social discount rate implied by the goods multiplier.
...
\end{align*}
so that the costate condition for $m^K=\lambda q$ reads $rq=F_K\left(1-\zeta\Omega^Y\right)-\delta+\Phi^I\delta\left(p^W-p^M-\zeta\right)+\dot q$. The third term equals $\delta\left(1-q\right)$ by the investment condition~\eqref{eq:sp:foc:I}, so the entire materials block collapses into $q$ and
\begin{align}
...
\end{align*}
The current-value costate is $m^M=-\lambda p^M$, and the costate condition reduces to
\begin{align}
...
In particular $p^M$ is a forward-weighted average of future values of $p^W$. It therefore shares the sign of $p^W$ whenever $p^W$ does not change sign over $\left[t,\infty\right)$, but not otherwise. With $p^W$ constant,
\begin{align}
...
\smalltitle{The waste stock.} A marginal tonne in the stockpile is drawn for handling at rate $\mu$. When handled, it absorbs $c^W$ final goods of collection and treatment, grossed up for their embodied material; a fraction $\alpha\varpi$ returns to production as secondary material, worth $\Psi-p^W$ per tonne net of the waste it will regenerate; and the remainder reaches the environment as the emission flow prices it. Since $\mathcal W$ acts only through $H=\mu\mathcal W$, every term in $\partial\mathcal L/\partial\mathcal W$ is proportional to $\mu$, and collecting them gives $\partial\mathcal L/\partial\mathcal W=\lambda\mu\left(h-p^{\mathcal W}\right)$ with the \emph{handling dividend}
\begin{align}
h &\equiv
...
\begin{align}
\dot p^{\mathcal W} &= \left(r+\mu\right)p^{\mathcal W}-\mu h,
&&\text{hence}
&
p^{\mathcal W}\left(t\right) &= \int_t^\infty \mu\,h\left(s\right)e^{-\int_t^s\left(r+\mu\right)d\tau}\,ds .
...
Three remarks. First, the forward representation makes the anthropogenic reserve the exact mirror of the virgin reserve: compare~\eqref{eq:sp:costate:S}. $S$ pays a dividend by making the extraction still to come cheaper; $\mathcal W$ pays one by delivering handled tonnes worth $h$ each. On squeeze paths, where material values grow while handling costs stay bounded, $h>0$ and the stockpile is a genuine asset --- the same economics an instantaneous-handling model expresses through a negative waste price, now attached to a stock that exists.

Second, the effective discount rate is smaller than it looks. Substituting $p^W=-p^{\mathcal W}$ from~\eqref{eq:sp:foc:W} into term (a) and collecting the $p^{\mathcal W}$ terms,
\begin{align}
\dot p^{\mathcal W} &= \left[r+\mu\left(1-\alpha\varpi\right)\right]p^{\mathcal W}
-\mu\Big[\alpha\varpi\,\Psi-c^W\left(1+\zeta\Omega^W\right)-p^P\left[1-\varpi\left(1-d^W\right)-d^W\alpha\varpi\right]\Big],
\label{eq:sp:costate:Wstock-net}
\end{align}
so in marginal-value terms the stockpile runs off only at the \emph{leakage} rate $\mu\left(1-\alpha\varpi\right)$, not at the handling rate $\mu$: the fraction $\alpha\varpi$ of each handled tonne re-enters production and eventually returns to the stockpile as waste, and the identification $p^W=-p^{\mathcal W}$ credits that return automatically.

Third, the instantaneous-handling limit recovers the static waste margin. With $h$ bounded and $\mu\to\infty$ --- or exactly, at any rest point of the price system --- \eqref{eq:sp:costate:Wstock} gives $p^{\mathcal W}=\mu h/\left(r+\mu\right)\to h$, and $p^W=-h$ rearranges to
\begin{align}
...
\end{align}
which is precisely the waste condition a model without a waste stock would impose at every instant. The stock changes \emph{when} the waste price is paid, not what it prices in steady state.
